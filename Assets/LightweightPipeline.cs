...
namespace UnityEngine.Experimental.Rendering.LightweightPipeline
{
    ...
    public static class CameraRenderTargetID
    {
        ...
        //Occlusion ADD
        public static int occMask;
        public static int occTemp1;
        public static int occTemp2;
    }

    public class LightweightPipeline : RenderPipeline

    {
        public static List<Renderer> customOccluders = new List<Renderer>();
        ...
        private RenderTargetIdentifier m_CustomOcclusionMask;
        private RenderTargetIdentifier m_CustomOcclusionTemp1;
        private RenderTargetIdentifier m_CustomOcclusionTemp2;
        ...
        public static readonly ShaderPassName customObjectDepthBack = new ShaderPassName("CustomObjectDepthBack");
        public static readonly ShaderPassName customObjectDepthFront = new ShaderPassName("CustomObjectDepthFront");
        ...
        public LightweightPipeline(LightweightPipelineAsset asset)
        {
            ...
            //Occlusion ADD
            CameraRenderTargetID.occMask = Shader.PropertyToID("_CustomOcclusionMask");
            CameraRenderTargetID.occTemp1 = Shader.PropertyToID("_CustomOcclusionTemp1");
            CameraRenderTargetID.occTemp2 = Shader.PropertyToID("_CustomOcclusionTemp2");

            m_CustomOcclusionTemp1 = new RenderTargetIdentifier(CameraRenderTargetID.occTemp1);
            m_CustomOcclusionTemp2 = new RenderTargetIdentifier(CameraRenderTargetID.occTemp2);
            m_CustomOcclusionMask = new RenderTargetIdentifier(CameraRenderTargetID.occMask);
            ...
            foreach (Camera camera in cameras)
            {
                ...
                if(!sceneViewCamera) {
                    if(LightweightUtils.HasFlag(frameRenderingConfiguration, FrameRenderingConfiguration.DepthPrePass)) {
                        ...
                        //cmd.SetGlobalTexture("_CustomDepth", m_DepthRT);
                    }
                }

                #region old

                //Shader.SetGlobalTexture("_CustomDepth", m_DepthRT);


                //var cmd2 = CommandBufferPool.Get("AfterDepth");
                //cmd2.name = "-------- After Depth";
                ////cmd2.BeginSample("Test Sample"); //shows it in the profiler
                ////cmd2.EndSample("Test Sample");
                //cmd2.ReleaseTemporaryRT(CameraRenderTargetID.depthCopy);
                //cmd2.SetGlobalInt("GlobalInt", 134);

                //context.ExecuteCommandBuffer(cmd2);
                //CommandBufferPool.Release(cmd2);

                //after this we have depth? so we should be able to render anyting in the forwardPass with access to this depth?
                //create a new render texture to use as our final screenspace mask (cleared to black to start?)
                //grab a list of all the "occluder" taged objects
                //for each one of them render their backface depth to a temp texture
                //render their frontface depth to a temp texture
                //if backfaceDepth < depthMaskDepth < frontfaceDepth  (//this means, is the pixel "depth" within the volume of the occluder.  Doesn't work with hollow occluders or concave etc..
                //   this pixel should be marked "masked" (set to white)

                //once all objects are done, set this rendertexture as a global shader variable so we can access it in the 
                //next forward pass for all rendering objects?

                //lets try getting a single object that will render to this pass ONLY (be invisible in every other pass)
                //render it to a rt, and set it to _CustomDepth to see if it displays.

                //CommandBuffer.DrawRenderer(renderer, material, submshIndex, shaderPass);

                //clear custom object temp textures

                //    CommandBuffer cmd2 = CommandBufferPool.Get("CustomObjectDepth");
                ////cmd.GetTemporaryRT()
                //    //cmd2.GetTemporaryRT(CameraRenderTargetID.occTemp1, -1, -1);
                //    SetRenderTarget(cmd2, m_CustomOcculsionTemp1, ClearFlag.All);

                //    context.ExecuteCommandBuffer(cmd2);
                //    CommandBufferPool.Release(cmd2);
                //CommandBuffer cd = CommandBufferPool.Get("CustomObjectDepth");
                ////SetRenderTarget()
                //SetRenderTarget(cd, m_CustomOcculsionTemp1, ClearFlag.All);
                ////cd./*SetRenderTarget*/(m_CustomOcculsionTemp1);
                //cd.ClearRenderTarget(true, true, Color.green, 0f);
                //context.ExecuteCommandBuffer(cd);
                //CommandBufferPool.Release(cd);

                //var occlusionDrawSettings = new DrawRendererSettings(m_CurrCamera, new ShaderPassName("CustomObjectDepth"));
                //occlusionDrawSettings.sorting.flags = SortFlags.CommonOpaque;

                //var occlusionFilterSettings = new FilterRenderersSettings(true) {
                //    renderQueueRange = RenderQueueRange.all
                //};

                //StartStereoRendering(ref context, frameRenderingConfiguration);

                //context.DrawRenderers(m_CullResults.visibleRenderers, ref occlusionDrawSettings, occlusionFilterSettings);

                //StopStereoRendering(ref context, frameRenderingConfiguration);



                ////cmd.SetGlobalTexture("_CustomDepth", m_CustomOcculsionTemp1);


                ////cmd.SetGlobalTexture("_CustomDepth", objectOnly);
                /////

                ////this IS the proper injection point, 
                ////but the sphere is never rendering to the texture...?
                //CommandBuffer cdd = CommandBufferPool.Get("CustomObjectDepthDone");
                ////cdd.SetRenderTarget(m_CustomOcclusionTemp2);
                ////cdd.ClearRenderTarget(true, true, Color.green);
                //cdd.SetGlobalTexture("_CustomObjectDepth", m_CustomOcculsionTemp1);

                //context.ExecuteCommandBuffer(cdd);

                //CommandBufferPool.Release(cdd);
                #endregion


                OcclusionDepth(ref context,  frameRenderingConfiguration);
                ...
                cmd.ReleaseTemporaryRT(CameraRenderTargetID.occTemp1);
        ...
        /// <summary>
        /// Renders all visible objects wit "customObjectDepthBack/Front" passes
        /// to separate back and front face temp render textures
        /// after we combine them to compute the volumetric mask texture
        /// </summary>
        /// <param name="context"></param>
        /// <param name="frameRenderingConfiguration"></param>
        private void OcclusionDepth(ref ScriptableRenderContext context, FrameRenderingConfiguration frameRenderingConfiguration) {
            //clear mask
            //Debug.Log("Number of occluders: " + OcclusionManager.instance.occlusionRenderers.Count);
            //Debug.Log("Num occluders: " + customOccluders.Count);
            CommandBuffer clearMaskBuf = CommandBufferPool.Get("Occlusion ClearMaskBuff");
            clearMaskBuf.SetRenderTarget(m_CustomOcclusionMask);
            clearMaskBuf.ClearRenderTarget(false, true, Color.black);
            context.ExecuteCommandBuffer(clearMaskBuf);
            CommandBufferPool.Release(clearMaskBuf); 

            for(int i = 0; i < customOccluders.Count; i++) {
                if(customOccluders[i] == null) continue;

                CommandBuffer cmd = CommandBufferPool.Get("Occlusion Depth Clear");
                ...
                CommandBuffer drawBack = CommandBufferPool.Get("Occlusion Depth Draw Back");
                drawBack.SetRenderTarget(m_CustomOcclusionTemp1);
                drawBack.DrawRenderer(customOccluders[i], customOccluders[i].material,0, 0);
                context.ExecuteCommandBuffer(drawBack);
                CommandBufferPool.Release(drawBack);

                //front
                CommandBuffer drawFront = CommandBufferPool.Get("Occlusion Depth Draw Front");
                drawFront.SetRenderTarget(m_CustomOcclusionTemp2);
                drawFront.DrawRenderer(customOccluders[i], customOccluders[i].material, 0, 1);
                context.ExecuteCommandBuffer(drawFront);
                CommandBufferPool.Release(drawFront);
                


                CommandBuffer setTemps = CommandBufferPool.Get("Occlusion Depth Set Temps");
                //c2.SetGlobalTexture("_CustomObjectDepth", CameraRenderTargetID.occTemp1);
                setTemps.SetGlobalTexture("_CustomObjectDepthTemp1", CameraRenderTargetID.occTemp1);
                setTemps.SetGlobalTexture("_CustomObjectDepthTemp2", CameraRenderTargetID.occTemp2);

                context.ExecuteCommandBuffer(setTemps);
                CommandBufferPool.Release(setTemps);

                //draw into mask using temp's (m_CustomOcclusionTemp1, m_CustomOcclusionTemp2) 
                CommandBuffer maskcmd = CommandBufferPool.Get("Occlusion Compute Object Volume");
                maskcmd.SetRenderTarget(m_CustomOcclusionMask);
                //we have our temp shader values, so we should be able to draw a material (that has access to our temps
                //to the mask texture?

                if(occlusionMaskMat == null) {
                    occlusionMaskMat = new Material(Shader.Find("OcclusionBlitCombineObjMasks"));
                }

                maskcmd.Blit(m_CustomOcclusionMask, m_CustomOcclusionMask, occlusionMaskMat);
                context.ExecuteCommandBuffer(maskcmd);
                CommandBufferPool.Release(maskcmd);

                CommandBuffer c2 = CommandBufferPool.Get("Occlusion Set Mask Done");
                c2.SetGlobalTexture("_CustomOcclusionMask", CameraRenderTargetID.occMask);
                context.ExecuteCommandBuffer(c2);
                CommandBufferPool.Release(c2);
            }

            #region old
            /*
            CommandBuffer cmd = CommandBufferPool.Get("Occlusion Depth Back");
                //SetRenderTarget(cmd, m_CustomOcclusionTemp1, m_CustomOcclusionTemp2, ClearFlag.All);
                cmd.SetRenderTarget(m_CustomOcclusionTemp1);
                cmd.ClearRenderTarget(true, true, Color.black);
                context.ExecuteCommandBuffer(cmd);
                ...
                var opaqueDrawSettings = new DrawRendererSettings(m_CurrCamera, customObjectDepthBack);
                ...
                //CommandBuffer cdd = CommandBufferPool.Get("Test2");
                //cdd.drasw
                StartStereoRendering(ref context, frameRenderingConfiguration);
                ...
                StopStereoRendering(ref context, frameRenderingConfiguration);
                //front
                CommandBuffer cmd2 = CommandBufferPool.Get("Occlusion Depth Front");
                //SetRenderTarget(cmd, m_CustomOcclusionTemp1, m_CustomOcclusionTemp2, ClearFlag.All);
                cmd2.SetRenderTarget(m_CustomOcclusionTemp2);
                cmd2.ClearRenderTarget(true, true, Color.black);
                context.ExecuteCommandBuffer(cmd2);
                CommandBufferPool.Release(cmd2);

                var opaqueDrawSettings2 = new DrawRendererSettings(m_CurrCamera, customObjectDepthFront);
                opaqueDrawSettings2.sorting.flags = SortFlags.CommonOpaque;


                var opaqueFilterSettings2 = new FilterRenderersSettings(true) {
                    renderQueueRange = RenderQueueRange.all
                };
                StartStereoRendering(ref context, frameRenderingConfiguration);
                context.DrawRenderers(m_CullResults.visibleRenderers, ref opaqueDrawSettings2, opaqueFilterSettings2);
                StopStereoRendering(ref context, frameRenderingConfiguration);

                //draw into mask using temp's (m_CustomOcclusionTemp1, m_CustomOcclusionTemp2) 
                CommandBuffer maskcmd = CommandBufferPool.Get("CustomObjectMaskCMD");
                //maskcmd.dr
                //maskcmd.DrawRenderer(renderer, maskcmd);

                //context.DrawRenderers()

                //prepares the shader variables for the "mask" pass
                CommandBuffer c2 = CommandBufferPool.Get("SetCustomObjectDepthTexture");
                c2.SetGlobalTexture("_CustomObjectDepth", CameraRenderTargetID.occTemp1);
                c2.SetGlobalTexture("_CustomObjectDepthTemp1", CameraRenderTargetID.occTemp1);
                c2.SetGlobalTexture("_CustomObjectDepthTemp2", CameraRenderTargetID.occTemp2);
        ...
        private void SetupIntermediateRenderTextures(CommandBuffer cmd, FrameRenderingConfiguration renderingConfig, bool shadows, int msaaSamples)
        {
            ...
            cmd.GetTemporaryRT(CameraRenderTargetID.occTemp1, colorRTDesc, FilterMode.Bilinear);
            cmd.GetTemporaryRT(CameraRenderTargetID.occTemp2, colorRTDesc, FilterMode.Bilinear);
            cmd.GetTemporaryRT(CameraRenderTargetID.occMask, colorRTDesc, FilterMode.Bilinear);
        }
